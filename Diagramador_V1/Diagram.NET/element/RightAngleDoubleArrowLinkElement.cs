...
{
	[Serializable]
	public class RightAngleDoubleArrowLinkElement: BaseLinkElement, IControllable, ILabelElement
	{
		...
		[NonSerialized]
		private RightAngleDoubleArrowLinkController controller;

		public RightAngleDoubleArrowLinkElement(ConnectorElement conn1, ConnectorElement conn2): base(conn1, conn2)
		{
			needCalcLink = true;
			InitConnectors(conn1, conn2);
			foreach(LineElement l in lines)
			{
				l.StartCap = LineCap.ArrowAnchor; //Flecha
				l.EndCap = LineCap.ArrowAnchor; //Flecha
			}
			startCap = LineCap.ArrowAnchor; //Flecha
			endCap = LineCap.ArrowAnchor; //Flecha
		...
		{
			if (controller == null)
				controller = new RightAngleDoubleArrowLinkController(this);
			return controller;
		}
